...
//-----------------------------------------------------------------------------
class CEmptyGameUI2ConVar : public ConVar
{
public:
	CEmptyGameUI2ConVar() : ConVar( "", "0" ) {}
...
static CEmptyGameUI2ConVar s_EmptyConVar;

// Helper for splitscreen ConVars
GameUI2ConVarRef::GameUI2ConVarRef( const char *pName )
{
	Init( pName, false );
}

GameUI2ConVarRef::GameUI2ConVarRef( const char *pName, bool bIgnoreMissing )
{
	Init( pName, bIgnoreMissing );
}

void GameUI2ConVarRef::Init( const char *pName, bool bIgnoreMissing )
{
	char pchName[ 256 ];
	...
	if ( !IsValid() )
	{
		...
		if ( g_pCVar || bFirst )
		{
			if ( !bIgnoreMissing )
			{
				Warning( "GameUI2ConVarRef %s doesn't point to an existing ConVar\n", pName );
			}
			bFirst = false;
		}
	}
...
GameUI2ConVarRef::GameUI2ConVarRef( IConVar *pConVar )
{
...
bool GameUI2ConVarRef::IsValid() const
{
