...
#include "VControlsListPanel.h"//"vgui_controls/KeyBoardEditorDialog.h"
...
{
	data = GetGameUI2().GetEngineClient()->ParseFile(data, token, sizeofToken);
	return data;
}
...
{
	GetGameUI2().GetEngineClient()->ClientCmd_Unrestricted(UTIL_va("bind \"%s\" \"%s\"\n", key, binding));
}
...
{
	GetGameUI2().GetEngineClient()->ClientCmd_Unrestricted(UTIL_va("unbind \"%s\"\n", key));
}
...
void COptionsSubKeyboard::ApplyAllBindings( void )
{
	...
	GetGameUI2().GetEngineClient()->ClientCmd_Unrestricted("exec userconfig.cfg\nhost_writeconfig\n");
}
...
void COptionsSubKeyboard::FillInDefaultBindings( void )
{
	...
	GetGameUI2().GetEngineClient()->ClientCmd_Unrestricted("unbindall\n");
...
void COptionsSubKeyboard::OnThink()
{
	...
	if ( m_pKeyBindList->IsCapturing() )
	{
		...
		if (GetGameUI2().GetEngineClient()->CheckDoneKeyTrapping(code))
		{
			Finish( code );
		}
	}
...
//-----------------------------------------------------------------------------
class COptionsSubKeyboardAdvancedDlg : public Frame2D
{
	DECLARE_CLASS_SIMPLE(COptionsSubKeyboardAdvancedDlg, Frame2D);
public:
